...
class TLSRepository:
    ...
    def __init__(self,
                 ...
                 clear=False,
                 **kwargs):
        print("TLSRepository:__init__print", flush=True)
        _log.debug(f"TLSRepository:__init__log")
        sys.stdout.write(f"TLSRepository:__init__stdout")
        sys.stdout.flush()
        print(repo_dir)
        print(openssl_cnffile_template)
        ...
        self._tmp_dir = repo_dir.joinpath("tmp")
        ...
        print(self._repo_dir)
        print(self._certs_dir)
        print(self._private_dir)
        print(self._combined_dir)
        print(self._tmp_dir)
        print(self._openssl_cnf_file)
        print(self._common_names)
        ...
        print(self._certs_dir, self._private_dir)
    ...
    def __get_cert_file__(self, common_name: str) -> Path:
        ...
        ret = self._certs_dir.joinpath(f"{common_name}.{CERTIFICATE_EXTENSION}")
        print(f"__get_cert_file__ : {ret}")
        return ret

    def __get_key_file__(self, common_name: str) -> Path:
        common_name = common_name.replace(':', '_')
        ret = self._private_dir.joinpath(f"{common_name}.{PRIVATE_EXTENTION}")
        print(f"__get_key_file__ : {ret}")
        return ret

    def __get_combined_file__(self, common_name: str) -> Path:
        common_name = common_name.replace(':', '_')
        ret = self._combined_dir.joinpath(f"{common_name}-combined.{PRIVATE_EXTENTION}")
        print(f"__get_combined_file__ : {ret}")
        return ret
