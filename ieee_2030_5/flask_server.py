    ...
    def make_environ(self):
        ...
        try:
            ...
            if PeerCertWSGIRequestHandler.is_admin(environ['PATH_INFO']):
                ...
                x509_text = self.connection.getpeercert(False)
                x509 = OpenSSL.crypto.load_certificate(OpenSSL.crypto.FILETYPE_ASN1, x509_binary)
                print ("x509_binary : ", x509_binary != None)
                print(x509_text)
...
def __build_ssl_context__(tlsrepo: TLSRepository) -> ssl.SSLContext:
    ...
    ca_cert = './all_certs.pem' #str(tlsrepo.ca_cert_file)
    ...
    ssl_context = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)  # ssl.create_default_context(ssl.Purpose.CLIENT_AUTH)
